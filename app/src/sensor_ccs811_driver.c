...
bool ccs811_driver_is_ready(void)
{
	...
	if (elapsed >= CCS811_CONDITIONING_TIME_MS) {
...
int64_t ccs811_driver_conditioning_time_remaining(void)
{
	...
	int64_t elapsed = k_uptime_get() - ccs811_init_time;
	int64_t remaining = CCS811_CONDITIONING_TIME_MS - elapsed;
...
int ccs811_driver_save_baseline(void)
{
	...
	if (baseline >= 0) {
		...
		/* TODO: Store in flash/settings for persistence across resets */
		return 0;
	} else {
...
bool ccs811_driver_baseline_save_due(void)
{
	...
	return (uptime - last_baseline_save) >= BASELINE_SAVE_INTERVAL_MS;
}
...
int ccs811_driver_manual_reset(void)
{
	...
	if (config->reset_gpio.port && config->wake_gpio.port) {
		...
		if (post_reset_result) {
			LOG_INF("CCS811 after manual reset: status=0x%02x, FW_MODE=%d, APP_VALID=%d",
			        post_reset_result->status,
			        (post_reset_result->status >> 7) & 1,
				    ...
				    float temp_celsius, float humidity_percent)
{
	...
	if (!isnan(temp_celsius) && !isnan(humidity_percent) &&
	    temp_celsius >= -40.0f && temp_celsius <= 85.0f &&
	    humidity_percent >= 0.0f && humidity_percent <= 100.0f) {

		temp_val.val1 = (int32_t)temp_celsius;
		temp_val.val2 = (int32_t)((temp_celsius - temp_val.val1) * 1000000);
		hum_val.val1 = (int32_t)humidity_percent;
		hum_val.val2 = (int32_t)((humidity_percent - hum_val.val1) * 1000000);
		...
		if (env_ret != 0) {
			...
			LOG_DBG("Updated CCS811 env data: %.1f°C, %.1f%%RH",
	...
	/* Switch to 60-second mode after first successful sample */
	if (!first_sample_obtained) {
		first_sample_obtained = true;
		int mode_ret = ccs811_mode_update(ccs811_dev, CCS811_MEASUREMENT_60SEC);
		if (mode_ret == 0) {
			LOG_INF("CCS811 switched to 60-second mode after first sample");
		} else {
			LOG_WRN("Failed to switch to 60-second mode: %d", mode_ret);
		}
	}
			       ...
			       float temp_celsius, float humidity_percent)
{
	...
	if ((ret == -EAGAIN || ret == -ETIME) && first_sample_obtained) {
		...
		const int64_t MAX_CACHE_AGE_MS = 5 * 60 * 1000; /* 5 minutes max age */
		...
		if (age_ms <= MAX_CACHE_AGE_MS) {
			...
			if (has_cached_data) {
				LOG_INF("Using cached CCS811 data (age: %lld ms): CO2=%d ppm, TVOC=%d ppb",
					age_ms, cached_co2_ppm, cached_tvoc_ppb);
				return 0;
			}
		} else {
			LOG_WRN("Cached CCS811 data too old (%lld ms), max age %lld ms",
