                          ...
                          void *buf, uint16_t len, uint16_t offset)
{
    ...
    return bt_gatt_attr_read(conn, attr, buf, len, offset,
                           &uptime_le, sizeof(uptime_le));
}

/* Uptime Service GATT definition */
BT_GATT_SERVICE_DEFINE(uptime_svc,
    BT_GATT_PRIMARY_SERVICE(BT_UUID_UPTIME_SERVICE),
...
int uptime_service_update(void)
{
    ...
    uint64_t old_uptime = current_uptime_seconds;
    current_uptime_seconds = k_uptime_get() / 1000;

    LOG_DBG("Uptime updated: %llu -> %llu seconds", old_uptime, current_uptime_seconds);
