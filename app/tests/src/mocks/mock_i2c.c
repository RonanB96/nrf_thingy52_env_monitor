...
static int process_mock_transaction(const struct mock_i2c_transaction *actual)
{
    if (mock_i2c_state.current_index >= mock_i2c_state.expected_count) {
        LOG_ERR("Unexpected I2C transaction: type=%d, addr=0x%04x",
                actual->type, actual->addr);
    ...
    zassert_equal(actual->type, expected->type,
                  "I2C transaction type mismatch at index %d", 
                  mock_i2c_state.current_index);
    zassert_equal(actual->addr, expected->addr,
                  "I2C address mismatch at index %d", 
                  mock_i2c_state.current_index);
    ...
    if (expected->read_len > 0 && actual->read_buf != NULL) {
        zassert_equal(actual->read_len, expected->read_len,
                      "I2C read length mismatch at index %d",
                      mock_i2c_state.current_index);
    ...
    LOG_DBG("Processed I2C transaction %d successfully", 
            mock_i2c_state.current_index - 1);
