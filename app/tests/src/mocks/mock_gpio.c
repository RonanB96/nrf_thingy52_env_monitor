...
void mock_gpio_reset(void)
{
    ...
    for (int i = 0; i < MAX_GPIO_DEVICES; i++) {
        memset(mock_gpio_state.devices[i].pins, 0, 
               sizeof(mock_gpio_state.devices[i].pins));
        mock_gpio_state.devices[i].device_ready = true;
    }
}
                                 ...
                                 gpio_flags_t expected_flags)
{
    ...
    struct mock_gpio_pin_state *pin_state = 
        &mock_gpio_state.devices[device_index].pins[pin];
    
    zassert_true(pin_state->configured, "GPIO pin %d was not configured", pin);
    zassert_equal(pin_state->flags, expected_flags,
                  "GPIO pin %d configuration mismatch. Expected: 0x%x, Actual: 0x%x",
                  pin, expected_flags, pin_state->flags);
                                     ...
                                     enum gpio_int_mode mode, enum gpio_int_trig trig)
{
    ...
    LOG_DBG("Mock GPIO interrupt configure: dev=%p, pin=%d, mode=%d, trig=%d", 
            dev, pin, mode, trig);
